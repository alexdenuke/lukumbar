...
let measurement = "%"; // measurement - измерение
let width = 100;
let position = width;
let activeSlide = 0;
...
let timerId = setInterval(rightMove, 5500);
...
function rightMove() {
  ...
  if (activeSlide == slides.length - 1) {
    ...
    activeSlide++;
    position += width;
    timerId = setInterval(rightMove, 5500);
  }
}
function leftMove() {
  clearInterval(timerId);
...
function returnStyles() {
  wrapSlides.style.transition = "all 0.6s ease";
  btnRight.onclick = rightMove;
  btnLeft.onclick = leftMove;
  timerId = setInterval(rightMove, 5500);
}
